    ...
    pub fn handle_keyboard_input(&mut self, event: KeyEvent) {
        if event.state.is_pressed() {
            ...
            if let Some(pty_master) = &self.pty_master {
                let cursor_keys_mode = self
                    .terminal
                    .as_ref()
                    .map(|t| t.cursor_keys_mode)
                    .unwrap_or(false);
