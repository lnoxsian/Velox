...
mod tests {
    ...
    fn test_tab_bar_visibility() {
        ...
        assert!(!tb.is_visible(1));
        assert!(tb.is_visible(2));
        assert!(tb.is_visible(5));

        tb.show_tab_bar = TabBarVisibility::Always;
        assert!(tb.is_visible(1));

        tb.show_tab_bar = TabBarVisibility::Never;
        assert!(!tb.is_visible(5));
    }

    #[test]
    fn test_tab_bar_hit_testing() {
        let tb = TabBar {
            show_tab_bar: TabBarVisibility::Always,
            show_close_button: true,
            show_new_tab_button: false,
            configured_height: Some(30.0),
            ..Default::default()
        };

        // Window width 800, 2 tabs -> tab_width is 400 (spans full 50% each, Terminator style)
        let tab_w = tb.tab_width(800.0, 2);
        assert_eq!(tab_w, 400.0);

        // Click in first tab
        let hit = tb.hit_test(50.0, 15.0, 800.0, 16, 2);
        assert_eq!(hit, TabBarHitResult::Tab(0));

        // Click on first tab's close button (tab 0 ends at 400, close button is ~376..396)
        let hit_close = tb.hit_test(385.0, 15.0, 800.0, 16, 2);
        assert_eq!(hit_close, TabBarHitResult::CloseTab(0));

        // Click in second tab
        let hit_tab1 = tb.hit_test(500.0, 15.0, 800.0, 16, 2);
        assert_eq!(hit_tab1, TabBarHitResult::Tab(1));

        // Click on second tab's close button (tab 1 ends at 800, close button is ~776..796)
        let hit_close1 = tb.hit_test(785.0, 15.0, 800.0, 16, 2);
        assert_eq!(hit_close1, TabBarHitResult::CloseTab(1));

        // Click outside tab bar (y > 30)
        let hit_outside = tb.hit_test(50.0, 35.0, 800.0, 16, 2);
